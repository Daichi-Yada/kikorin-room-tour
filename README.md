...
3種類のきこりんルームと普通の部屋を選べるコンセプト内覧です。現在の公開版と今後の修正元は、このGitHubリポジトリの `main` です。Blenderは使用しません。

## 2026-09-15 客室タイプと予約連携

- `?room=family`：きこりんファミリー。広めの客室、ベッド2台、動物の足跡・落ち葉・コウモリの飾りなど。中間発表13枚目を参照。
- `?room=premium`：きこりんビジネスプレミアム。既存の6地点を引き継ぎ、ドア前を空けた靴脱ぎスペースと室内用スリッパを追加。
- `?room=basic`：きこりんベーシック。狭めの客室、絨毯の床、木質シートの壁、白い天井。
- `?room=standard`：普通の部屋。参考写真による紹介。
- 3種類のきこりんルームに丸太枕・BAUM・ヒノキ／スギ／ヒバの香りスプレーを設置。WOOD CYCLE公式図、木材サンプル、客室ガイド、チェーンソー音を継続。
- ファミリー・ベーシックは各1地点の360°内覧で、下部の3つのボタンから見どころの方向へ切り替えます。
- 予約ページは `reservation/`。4種類から選ぶと、その客室の紹介・内覧へ遷移。内覧から予約に戻ると客室選択を引き継ぎます。氏名や日程はURLに含めません。
- `room-catalog.js` が名称・定員・サンプル料金・写真の共通データ。未指定・不明なIDはプレミアムに戻します。
- 画像は `assets/rooms/*-4k.jpg`（4096×2048）。プレミアムは旧4K画像を土台に備品と玄関を部分修正。ファミリーとベーシックは100°の6面を補修して全天球に再構成しています。継ぎ目や鏡、小物の細部には生成画像の不整合が残ります。

## 2026-09-15 品質・説明の改訂

- 3種類に分ける前の客室は [`legacy/`](legacy/) に保存（元コミット `57be1b9`）。元の6枚の4K画像と説明を維持し、現在版からリンクしています。
- きこりんの参照画像を使って顔を補修。玄関はドア前を空けた石調タイルとスリッパに変更。
- 室内の額に公式WOOD CYCLE画像を射影変換で直接掲載。額を押すと公式画像を拡大できます。
- `0915/tour-config.json` の手動説明を反映。UTF-16の原稿を読み込み、公開JSONをUTF-8に正規化。原稿は変更していません。重複句点・重複文を整理し、BAUMの木製パーツ（オーク）と成分（ヒノキ）を区別。睡眠の説明は木質内装と間接照明の研究に合わせました。
- 予約ページの紹介バナーと共通設備の宣伝を削除。選択欄の外にきこりんルームの名前や説明を出しません。
- 生成原本・投影面・プロンプトは公開ディレクトリ外の `project/assets/quality-20260915/` に保存。
- `tools/build_room_assets.py` で生成PNGからJPEG・予約用写真を作成。`tools/package_assets.py` で内覧のサムネイルと検査記録を更新。指示文は `docs/room-image-prompts.txt`、画像記録は `docs/room-asset-manifest.json`。
- 検証：`node --test tests/room-routing.test.cjs`。UIはPC・390px幅で、客室切り替え、4予約先、定員超過・日付不正、設備・香り・公式図・ガイドの操作を確認。

以下の9月12日の記録は過去版です。画像寸法・客室タイプは上記の9月15日の仕様を優先してください。
