...
- `?room=premium`：きこりんビジネスプレミアム。既存の6地点を引き継ぎ、靴を脱ぐ玄関・靴収納を追加。
...
- 画像は `assets/rooms/`。今回は built-in image_gen の1774×887出力を使用。旧4K画像はルートに残していますが、現在の内覧では使用しません。継ぎ目や鏡、小物の細部には生成画像の不整合が残ります。
