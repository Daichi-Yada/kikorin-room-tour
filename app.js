...
function openBrochure(){closeAlarm();$('#detail').hidden=true;if(!$('#brochure-frame').getAttribute('src'))$('#brochure-frame').src='brochure.html?v=rooms-20260915';$('#brochure-dialog').showModal();}
...
async function start(){
 ...
 try{
  ...
  const res=await fetch((window.activeRoomId==='premium'?(simple?'tour-config-equirect.json':'tour-config.json'):'tour-config-'+window.activeRoomId+'.json')+'?v=rooms-20260915');if(!res.ok)throw Error('設定を読み込めません');
  const config=await res.json();
  const surfacesResponse=await fetch('surface-config.json?v=rooms-20260915');
  ...
  if(window.activeRoomId==='premium')window.createRoomSurfaces(viewer,surfaces,openBrochure);
