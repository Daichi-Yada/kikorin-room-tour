...
Chromeで全6地点のラベル・表紙位置、デスクでの拡大、パンフレットの開閉を確認。390×844のスマートフォン幅でも本文を確認し、横方向のはみ出しがないことを検査。日本語の文章チェックを実行し、不要な回りくどい表現を修正。JavaScript構文と両ツアー設定の一致を確認。公開版で読み込み中の地点変更による背景の不一致を検出したため、地点ボタンは画像の読み込み完了まで無効にする。
