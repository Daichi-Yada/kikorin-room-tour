"""Project and stitch AI-edited perspective patches while retaining the 4K base."""
...
work=root.parent/'assets/quality-20260915'
config=json.loads((root/'legacy/tour-config.json').read_text())
entry=[(180,-35,95),(-146,-28,75),(178,-20,70),(-150,-26,70),(-178,-17,55),(-166,-15,55)]
def basis(yaw,pitch):
...
def prepare():
 ...
 for i,(sid,scene) in enumerate(config['scenes'].items()):
  source=root/(sid+'.jpg');arr=np.asarray(Image.open(source));h,w=arr.shape[:2]
  feats={s.get('feature'):s for s in scene['hotSpots']}
  for kind,yaw,pitch,fov in [('bed',feats['kikorin']['yaw'],feats['kikorin']['pitch'],65),('desk',feats['baum']['yaw'],feats['baum']['pitch'],60),('entry',*entry[i])]:
   ...
   jobs.append(dict(name=name,scene=sid,kind=kind,yaw=yaw,pitch=pitch,fov=fov,path=str(path)))
 (work/'patches.json').write_text(json.dumps(jobs,indent=2))
 for kind in ['bed','desk','entry']:
  tiles=[Image.open(j['path']).resize((512,512)) for j in jobs if j['kind']==kind]
...
def stitch():
 jobs=json.loads((work/'patches.json').read_text())
 for sid in config['scenes']:
  base=np.asarray(Image.open(root/(sid+'.jpg')).convert('RGB'),dtype=np.float32);h,w=base.shape[:2]
  for j in sorted([j for j in jobs if j['scene']==sid],key=lambda j:['desk','bed','entry'].index(j['kind'])):
   ...
   for start in range(0,h,128):
    ...
    weight=np.clip((1-np.maximum(abs(dx),abs(dy))/t)/.14,0,1);weight=weight*weight*(3-2*weight);weight[dep<=0]=0
    px=(dx/t+1)*.5*(iw-1);py=(1-dy/t)*.5*(ih-1)
    if j['kind']=='bed':
     nx=px/(iw-1);ny=py/(ih-1);local=np.minimum.reduce([(nx-.15)/.08,(.82-nx)/.08,(ny-.22)/.08,(.84-ny)/.08]);weight*=np.clip(local,0,1)
    sampled=np.stack([map_coordinates(im[:,:,c],[py,px],order=1,mode='nearest') for c in range(3)],axis=-1)
    base[start:stop]=base[start:stop]*(1-weight[:,:,None])+sampled*weight[:,:,None]
  name='premium' if sid=='PANO_01_ENTRANCE' else 'premium-'+sid[5:]
